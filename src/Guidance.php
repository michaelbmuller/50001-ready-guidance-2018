...
class Guidance
{
    ...
    {
        foreach ($customTips as $task_id => $customTip) {
            $this->tasks[$task_id]->custom_tips = $customTip;
        }
    ...
    public function getTasksByISO($iso_section)
    {
        ...
        foreach ($this->tasks as $task) {
            if (\in_array($iso_section, $task->relatedIsoSections, true)) {
                $matchingTasks[] = $task;
            }
        }
    ...
    {
        $file_contents = file_get_contents(__DIR__ . '/../guidance/task_structure.txt');
        $currentTask = null;
        ...
        foreach (explode(PHP_EOL, $file_contents) as $row) {
            ...
            if ('Section' === $items[0]) {
                $currentSectionCode = Support::ConvertSectionName($items[1]);
            }
            if ('Task' === $items[0]) {
            ...
            if ('Prerequisite Tasks' === $items[0]) {
                $prerequisites = [];
                if (trim($items[1])) {
                    $prerequisites = array_map('trim', explode(',', trim($items[1])));
                }
                $this->tasks[$currentTask]->prerequisites = $prerequisites;
                foreach ($prerequisites as $prerequisite) {
                    if ($prerequisite < 1 || $prerequisite > 25) {
                        throw new \Exception('Prerequisite Task ID not valid ' . $prerequisite, 404);
                    }
                    $this->tasks[$prerequisite]->leadsTo[$currentTask] = $currentTask;
                }
            }
            if ('Level of Effort' === $items[0]) {
                $this->tasks[$currentTask]->effort = trim($items[1]);
            }
            if ('Related ISO Sections' === $items[0]) {
                $this->tasks[$currentTask]->relatedIsoSections = array_map('trim', explode(',', $items[1]));
            }
        }
    }
    ...
    protected function loadSectionTasks()
    {
        ...
        foreach ($this->taskIDsBySection as $sectionCode => $task_ids) {
            foreach ($task_ids as $task_id) {
                $this->tasksBySection[$sectionCode][$task_id] = $this->tasks[$task_id];
            }
        }
    ...
    protected function loadResources()
    {
        ...
        foreach ($this->resources as $resource) {
            foreach ($resource->associatedTasks as $associatedTaskID) {
                if ($associatedTaskID < 1 || $associatedTaskID > 25) {
